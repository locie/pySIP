...
model_list = [
    ...
    TwTiTm_RoRiRmAwAicv(par_rc3, latent_forces='Qv') <= QuasiPeriodic32(parameters=par_qp),
]
...
def test_disc_LTI(model, random_dt):
    ...
    dQ = np.array([model.dQ[n] for n, f in zip(model._names, model.parameters.free) if f])
    ...
    tic = time()
    Qd_lyap_bis, dQd_lyap = disc_d_diffusion_lyap(model.A, QQ, ssm.A[0], dA, dQQ, dssm.dA[0])
    toc = time() - tic
    print(f'lyapunov: {toc:.4f}')

    tic = time()
    Qd_kron_bis, dQd_kron = disc_d_diffusion_kron(model.A, QQ, ssm.A[0], dA, dQQ, dssm.dA[0])
    toc = time() - tic
    print(f'kron: {toc:.4f}')
    ...
    if isinstance(model, GPModel):
        ...
        dP0 = np.array([model.dP0[n] for n, f in zip(model._names, model.parameters.free) if f])
        dPinf = dP0.swapaxes(1, 2) @ model.P0 + model.P0.T @ dP0

        Qd_statio = disc_diffusion_stationary(Pinf, ssm.A[0])
        Qd_statio_bis, dQd_statio = disc_d_diffusion_stationary(Pinf, ssm.A[0], dPinf, dssm.dA[0])
