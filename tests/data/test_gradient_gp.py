...
def data_Periodic():
    ...
    t, y = generate_sine(
        n=50, bounds=(0, 2), period=1.0, amplitude=1.0, random=True, noise_std=0.3
    )
    return pd.DataFrame(index=t, data=y, columns=["y"])
...
def generate_regressor_product(*statespaces):
    ...
    for ssm in statespaces:
        yield Regressor(
            ss=Periodic(parameters=par_Periodic) * ssm(parameters=par_Matern)
        )
...
@pytest.mark.parametrize(
    "regressor", generate_regressor_sum(Matern12, Matern32, Matern52)
)
...
@pytest.mark.parametrize(
    "regressor", generate_regressor_product(Matern12, Matern32, Matern52)
)
...
@pytest.mark.parametrize(
    "models", [[Matern12, QuasiPeriodic12], [Matern32, QuasiPeriodic32]]
)
