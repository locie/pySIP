...
def test_fit_hmc_m32():
    ...
    fit = reg.fit(
        df=df,
        outputs="y",
        options={"init": "fixed", "n_cpu": n_cpu, "dense_mass_matrix": True},
    )
    # return df, reg, fit
    ...
    assert np.mean(
        np.mean((df["y"].values - xm[:, :, 0]) ** 2, axis=1) ** 0.5
    ) == pytest.approx(5.8e-2, rel=5e-2)

    ym, ysd = reg.posterior_predictive(
        trace=fit.posterior, df=df, outputs="y", n_cpu=n_cpu
    )
