...
class TwTiTm_RoRiRmAwAi(RCModel):
    ...
    def update_state_space_model(self):
        (Ro, Ri, Rm, Cw, Ci, Cm, Aw, Ai, sigw_w, sigw_i, sigw_m, sigv, x0_w, x0_i, x0_m, sigx0_w, sigx0_i, sigx0_m, *_) = self.parameters.theta

        self.A[:] = [
            [-(Ro + Ri) / (self.sC * Cw * Ri * Ro), 1.0 / (self.sC * Cw * Ri), 0.0],
            [1.0 / (self.sC * Ci * Ri), -(Ri + Rm) / (self.sC * Ci * Ri * Rm), 1.0 / (self.sC * Ci * Rm)],
            [0.0, 1.0 / (self.sC * Cm * Rm), -1.0 / (self.sC * Cm * Rm)]
        ]

        self.B[:2, :] = [
            [1.0 / (self.sC * Cw * Ro), Aw / (self.sC * Cw), 0.0],
            [0.0, Ai / (self.sC * Ci), 1.0 / (self.sC * Ci)]
        ]
    ...
    def update_jacobian(self):
        (Ro, Ri, Rm, Cw, Ci, Cm, Aw, Ai, *_) = self.parameters.theta
        ...
        self.dA['Ci'][1, :] = [-1.0 / (self.sC * Ri * Ci**2), (Ri + Rm) / (self.sC * Ri * Rm * Ci**2), -1.0 / (self.sC * Rm * Ci**2)]

        self.dA['Cm'][2, 1:] = [-1.0 / (self.sC * Rm * Cm**2), 1.0 / (self.sC * Rm * Cm**2)]

        self.dB['Ro'][0, 0] = -1.0 / (self.sC * Cw * Ro**2)

        self.dB['Cw'][0, :2] = [-1.0 / (self.sC * Ro * Cw**2), -Aw / (self.sC * Cw**2)]

        self.dB['Ci'][1, 1:] = [-Ai / (self.sC * Ci**2), -1.0 / (self.sC * Ci**2)]

        self.dB['Aw'][0, 1] = 1.0 / (self.sC * Cw)

        self.dB['Ai'][1, 1] = 1.0 / (self.sC * Ci)
