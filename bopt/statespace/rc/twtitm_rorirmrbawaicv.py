...
class TwTiTm_RoRiRmRbAwAicv(RCModel):
    ...
    def update_state_space_model(self):
        (Ro, Ri, Rm, Rb, Cw, Ci, Cm, Aw, Ai, cv, sigw_w, sigw_i, sigw_m, sigv, x0_w, x0_i, x0_m, sigx0_w, sigx0_i, sigx0_m, *_) = self.parameters.theta
        ...
        self.B[:2, :] = [
            ...
            [0.0, 1.0 / (self.sC * Ci * Rb), Ai / (self.sC * Ci), 1.0 / (self.sC * Ci), cv / (self.sC * Ci)]
        ]
    ...
    def update_jacobian(self):
        ...
        self.dB['Ci'][1, 1:] = [
            -1.0 / (self.sC * Rb * Ci**2),
            -Ai / (self.sC * Ci**2),
            -1.0 / (self.sC * Ci**2), -cv / (self.sC * Ci**2)
        ]
