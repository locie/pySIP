...
class TwTiTb_RwRbAwAicv(RCModel):
    ...
    def update_state_space_model(self):
        (Rw, Rb, Cw, Ci, Cb, Aw, Ai, cv, sigw_w, sigw_i, sigw_b, sigv, x0_w, x0_i, x0_b, sigx0_w, sigx0_i, sigx0_b, *_) = self.parameters.theta

        self.A[:] = (
            [-4.0 / (self.sC * Cw * Rw), 2.0 / (self.sC * Cw * Rw), 0.0],
            [2.0 / (self.sC * Ci * Rw), -2.0 * (Rb + Rw) / (self.sC * Ci * Rb * Rw), 2.0 / (self.sC * Ci * Rb)],
            [0.0, 2.0 / (self.sC * Cb * Rb), -4.0 / (self.sC * Cb * Rb)]
        )

        self.B[:] = [
            [2.0 / (self.sC * Cw * Rw), 0.0, Aw / (self.sC * Cw), 0.0, 0.0],
            [0.0, 0.0, Ai / (self.sC * Ci), 1.0 / (self.sC * Ci), cv / (self.sC * Ci)],
            [0.0, 2.0 / (self.sC * Cb * Rb), 0.0, 0.0, 0.0]
        ]
    ...
    def update_jacobian(self):
        ...
        self.dA['Rb'][1:, 1:] = [
            (2.0 / (self.sC * Ci * Rb**2), -2.0 / (self.sC * Ci * Rb**2)),
            (-2.0 / (self.sC * Cb * Rb**2), 4.0 / (self.sC * Cb * Rb**2))
        ]

        self.dA['Cw'][0, :2] = [4.0 / (self.sC * Rw * Cw**2), -2.0 / (self.sC * Rw * Cw**2)]

        self.dA['Ci'][1, :] = [
            -2.0 / (self.sC * Rw * Ci**2),
            2.0 * (Rb + Rw) / (self.sC * Rb * Rw * Ci**2),
            -2.0 / (self.sC * Rb * Ci**2)
        ]
        ...
        self.dB['Rb'][2, 1] = -2.0 / (self.sC * Cb * Rb**2)

        self.dB['Cw'][0, :3] = [-2.0 / (self.sC * Rw * Cw**2), 0.0, -Aw / (self.sC * Cw**2)]

        self.dB['Ci'][1, 2:] = [-Ai / (self.sC * Ci**2), -1.0 / (self.sC * Ci**2), -cv / (self.sC * Ci**2)]

        self.dB['Cb'][2, 1] = -2.0 / (self.sC * Rb * Cb**2)
