...
class TwTiTb_RoRiRbAwAicv(RCModel):
    ...
    def update_state_space_model(self):
        (Ro, Ri, Rb, Cw, Ci, Cb, Aw, Ai, cv, sigw_w, sigw_i, sigw_b, sigv, x0_w, x0_i, x0_b, sigx0_w, sigx0_i, sigx0_b, *_) = self.parameters.theta

        self.A[:] = [
            [-(Ro + Ri) / (self.sC * Cw * Ri * Ro), 1.0 / (self.sC * Cw * Ri), 0.0],
            [1.0 / (self.sC * Ci * Ri), -(Rb + 2.0 * Ri) / (self.sC * Ci * Rb * Ri), 2.0 / (self.sC * Ci * Rb)],
    ...
    def update_jacobian(self):
        ...
        self.dA['Cw'][0, :2] = [
            (Ro + Ri) / (self.sC * Ri * Ro * Cw**2),
            -1.0 / (self.sC * Ri * Cw**2)
        ]

        self.dA['Ci'][1, :] = [
            -1.0 / (self.sC * Ri * Ci**2),
            (Rb + 2.0 * Ri) / (self.sC * Rb * Ri * Ci**2),
            -2.0 / (self.sC * Rb * Ci**2)
        ]

        self.dA['Cb'][2, 1:] = [-2.0 / (self.sC * Rb * Cb**2), 4.0 / (self.sC * Rb * Cb**2)]
