...
@dataclass
class TwTiTm_RoRiRmAwAicv(RCModel):
    ...
    def update_state_space_model(self):
        (Ro, Ri, Rm, Cw, Ci, Cm, Aw, Ai, cv, sigw_w, sigw_i, sigw_m, sigv, x0_w, x0_i, x0_m, sigx0_w, sigx0_i, sigx0_m, *_) = self.parameters.theta

        self.A[:] = [
            [-(Ro + Ri) / (self.sC * Cw * Ri * Ro), 1.0 / (self.sC * Cw * Ri), 0.0],
            [
                1.0 / (self.sC * Ci * Ri),
                -(Ri + Rm) / (self.sC * Ci * Ri * Rm),
                1.0 / (self.sC * Ci * Rm),
            ],
        ...
        self.B[:2, :] = [
            [1.0 / (self.sC * Cw * Ro), Aw / (self.sC * Cw), 0.0, 0.0],
    ...
    def update_jacobian(self):
        (Ro, Ri, Rm, Cw, Ci, Cm, Aw, Ai, cv, *_) = self.parameters.theta
