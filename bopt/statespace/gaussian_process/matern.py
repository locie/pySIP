...
from bopt.statespace.base import GPModel
...
class Matern12(GPModel):
    ...
    The Matérn covariance function with smoothness parameter, nu=1/2
    is equal to the exponential covariance function.
    ...
    def update_jacobian(self):
        ...
        self.dQ["lscale"][:] = -2.0**0.5 / 2.0 * mscale * lscale**(-1.5)
...
class Matern52(GPModel):
    ...
    def update_state_space_model(self):
        ...
        self.P0[:] = ((mscale, 0.0, -tmp),
                      (0.0, tmp, 0.0),
                      (-tmp, 0.0, 5.0 * mscale / lscale**2))
    ...
    def update_jacobian(self):
        ...
        self.dA["lscale"][2, :] = (15.0 * 5.0 * 0.5 / lscale**4,
                                   30.0 / lscale**3,
        ...
        self.dP0["mscale"][:] = ((1.0, 0.0, -tmp / mscale),
                                 (0.0, tmp / mscale, 0.0),
                                 (-tmp / mscale, 0.0, 5.0 / lscale**2))

        self.dP0["lscale"][:] = ((0.0, 0.0, tmp / lscale),
                                 (0.0, -tmp / lscale, 0.0),
                                 (tmp / lscale, 0.0,
                                  -10.0 * mscale / lscale**3))
