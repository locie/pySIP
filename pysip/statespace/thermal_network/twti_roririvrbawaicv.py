...
class TwTi_RoRiRivRbAwAicv(RCModel):
    ...
    def update_continuous_ssm(self):
        ...
        self.A[:] = [
            ...
            [1.0 / (Ci * Ri), -(Rb * Ri + Rb * Riv + Ri * Riv) / (Ci * Rb * Ri * Riv)],
        ]
        self.B[:] = [
            [1.0 / (Cw * Ro), 0.0, Aw / Cw, 0.0, 0.0],
            [1.0 / (Ci * Riv), 1.0 / (Ci * Rb), Ai / Ci, 1.0 / Ci, cv / Ci],
        ]
    ...
    def update_continuous_dssm(self):
        ...
        self.dA['Riv'][1, 1] = 1.0 / (Ci * Riv ** 2)
        self.dA['Rb'][1, 1] = 1.0 / (Ci * Rb ** 2)
        self.dA['Cw'][0, :] = [(Ro + Ri) / (Ri * Ro * Cw ** 2), -1.0 / (Ri * Cw ** 2)]
        self.dA['Ci'][1, :] = [
            -1.0 / (Ri * Ci ** 2),
            (Rb * Ri + Rb * Riv + Ri * Riv) / (Rb * Ri * Riv * Ci ** 2),
        ]

        self.dB['Ro'][0, 0] = -1.0 / (Cw * Ro ** 2)
        self.dB['Riv'][1, 0] = -1.0 / (Ci * Riv ** 2)
