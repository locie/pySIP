...
class TwTi_RoRiA(RCModel):
    ...
    def update_continuous_ssm(self):
        ...
        self.A[:] = [
            [-(1.0 / Ro + 1.0 / Ri) / Cw, 1.0 / (Cw * Ri)],
            [1.0 / (Ci * Ri), -1.0 / (Ci * Ri)],
        ]
        self.B[:] = [[1.0 / (Cw * Ro), A / Cw, 0.0], [0.0, (1.0 - A) / Ci, 1.0 / Ci]]
    ...
    def update_continuous_dssm(self):
        ...
        self.dA['Cw'][0, :] = [(1.0 / Ro + 1.0 / Ri) / (Cw ** 2), -1.0 / (Cw ** 2 * Ri)]
        self.dA['Ci'][1, :] = [-1.0 / (Ci ** 2 * Ri), 1.0 / (Ci ** 2 * Ri)]

        self.dB['Ro'][0, 0] = -1.0 / (Cw * Ro ** 2)
        self.dB['Cw'][0, :2] = [-1.0 / (Cw ** 2 * Ro), -A / (Cw ** 2)]
        self.dB['Ci'][1, 1:] = [-(1.0 - A) / (Ci ** 2), -1.0 / (Ci ** 2)]
        self.dB['A'][:, 1] = [1.0 / Cw, -1.0 / Ci]
