    ...
    .. tikz::
        ...
        if self.__tikz_mode__ == "spring":
            if isinstance(self.__tikz__, dict):
                pos = {
                    k: v
                    for k, v in self.__tikz__.items()
                    if k in self.inputs + self.outputs + self.nodes
                }
                fixed = pos.keys() if pos else None
            else:
                pos, fixed = None, None
            layout = nx.drawing.layout.spring_layout(
                G, scale=self.__tikz_scale__, pos=pos, fixed=fixed
            )
        else:
        ...
        for start, end, metadata in self.edges:
            ...
            if (
                isinstance(self.__tikz__, dict)
                and metadata in self.__tikz__
                and metadata not in self.inputs + self.outputs + self.nodes
            ):
