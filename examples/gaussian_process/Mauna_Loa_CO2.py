...
# values equal to -99.99 are missing data
df.drop(df.index[df['CO2'] == -99.99], inplace=True)
...
# fit
out = reg.fit(df=df, outputs='CO2_fit')
...
axes.plot(df.index, df['CO2'], '--r', lw=2, label='data')
axes.plot(tnew, C02_mean + ym, color='navy', lw=2, label='prediction mean')
axes.fill_between(
    tnew,
    C02_mean + ym - 1.96 * ysd,
    C02_mean + ym + 1.96 * ysd,
    color='darkblue',
    alpha=0.2,
    label=r'95% CI',
...
axes.set_ylabel('CO2 [ppm]', fontsize=14)
axes.legend(loc='best', fancybox=True, framealpha=0.5)
sns.despine()
