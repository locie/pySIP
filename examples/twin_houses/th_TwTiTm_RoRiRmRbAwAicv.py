...
---------------
  ...
  - Np: 15
  - fit: ok
  - log-lik fit: -4039.824376
  - log-lik pred: -2844.10880029
"""

import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
from bopt.statespace.rc import TwTiTm_RoRiRmRbAwAicv
...
south_T_header.remove('living_GT')
...
y0 = y[0, 0] / 100.0
u = df[['Ambient temperature', 'north_T', 'Solar radiation: global horizontal',
        'south_H', 'vent2_H']][start_fit:stop_fit].values.T

dt = 600
t = np.arange(0, dt * y.shape[1], dt)

reg = Regressor(TwTiTm_RoRiRmRbAwAicv())

reg.ss.parameters.set_parameter(
    "Ro", value=5.53e-2, transform="lowup", bounds=(1e-8, 10.0))
reg.ss.parameters.set_parameter(
    "Ri", value=2.09e-3, transform="lowup", bounds=(1e-8, 1.0))
reg.ss.parameters.set_parameter(
    "Rm", value=2.31e-3, transform="lowup", bounds=(1e-8, 1.0))
reg.ss.parameters.set_parameter(
    "Rb", value=4.98e-3, transform="lowup", bounds=(1e-8, 1.0))
reg.ss.parameters.set_parameter(
    "Cw", value=3.12e-2, transform="lowup", bounds=(1e-8, 10.0))
reg.ss.parameters.set_parameter(
    "Ci", value=6.73e-3, transform="lowup", bounds=(1e-8, 1.0))
reg.ss.parameters.set_parameter(
    "Cm", value=1.38e-1, transform="lowup", bounds=(1e-8, 10.0))
reg.ss.parameters.set_parameter(
    "Aw", value=1.06, transform="lowup", bounds=(1e-8, 10.0))
reg.ss.parameters.set_parameter(
    "Ai", value=1.24, transform="lowup", bounds=(1e-8, 10.0))
reg.ss.parameters.set_parameter(
    "cv", value=1.00, transform="lowup", bounds=(1e-8, 10.0))
reg.ss.parameters.set_parameter(
    "sigw_w", value=1e-2, transform="lowup", bounds=(1e-8, 1.0))
reg.ss.parameters.set_parameter(
    "sigw_i", value=1e-2, transform="lowup", bounds=(1e-8, 1.0))
reg.ss.parameters.set_parameter(
    "sigw_m", value=1e-2, transform="lowup", bounds=(1e-8, 1.0))
reg.ss.parameters.set_parameter(
    "sigv", value=1e-2, transform="lowup", bounds=(1e-8, 1.0))
reg.ss.parameters.set_parameter(
    "x0_w", value=0.292, transform="lowup", bounds=(1e-8, 1.0))
reg.ss.parameters.set_parameter("x0_i", value=y0, transform="fixed")
reg.ss.parameters.set_parameter(
    "x0_m", value=0.295, transform="lowup", bounds=(1e-8, 1.0))
...
reg.ss.parameters.set_parameter("sigx0_m", value=1.0, transform="fixed")
...
P0p_m = np.sqrt(P[-1, 2, 2])
theta_fit = np.copy(reg.ss.parameters.theta)
theta_valid = np.copy(theta_fit)
theta_valid[-6:] = [x0p_w, x0p_i, x0p_m, P0p_w, P0p_i, P0p_m]
...
up = df[['Ambient temperature', 'north_T', 'Solar radiation: global horizontal',
         'south_H', 'vent2_H']][start_valid:stop_valid].values.T
...
dfp.plot(y='south_T', color='k', use_index=True)
